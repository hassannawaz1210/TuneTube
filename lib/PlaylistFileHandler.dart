...
import 'package:tunetube/MyAppBar.dart';
...
class PlaylistFileHandler {
  ...
  static Future<void> saveToPlaylist(
    ...
    try {
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        MySnackbar.show("Video added to playlist.")
      );
    } catch (e) {
  ...
  static void removeFromPlaylist(BuildContext context, Map<String, dynamic> video) async {
    try {
      File file;
      if (Platform.isAndroid) {
        // For Android, read the file from the application documents directory
      ...
      if (await file.exists()) {
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          MySnackbar.show("Video removed from playlist.")
        );
  ...
  // static Future<void> readPlaylist(final playlist) async {
  //   try {
  //     if (Platform.isAndroid) {
  //       // For Android, read the file from the application documents directory
  //       final directory = await getApplicationDocumentsDirectory();
  //       final file = File('${directory.path}/playlist.txt');
  //       await _readFile(file, playlist);
  //     } else if (Platform.isWindows || Platform.isLinux || Platform.isMacOS) {
  //       // For desktop, read the file from the project directory
  //       final file = File('lib/playlist.txt');
  //       await _readFile(file, playlist);
  ...
  // static Future<void> _readFile(File file, var playlist) async {
  //   if (await file.exists()) {
